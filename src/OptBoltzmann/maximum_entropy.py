...
def flux_ent_opt(obj_coefs: np.array,vcount_upper_bound: np.array,f_log_counts: np.array,S: np.array,K: np.array, 
    solver_opts: dict =dict(max_iter = 10000,
          max_cpu_time= 800000
          tol=1e-7,
          acceptable_tol=1e-6,
          linear_solver= 'mumps'),) -> list:
    ...
    :param vcount_upper_bound: Metabolite upper bounds.
    ...
    :param f_log_counts: Fixed Metabolite log counts.
    ...
    :param K: :math:`e^{-\frac{\Delta G^\circ}{RT}}`.
    - The equilibrium constants for each reaction
    - A vector with length equal to number of reactions
    :returns: y_sol, alpha_sol, n_sol
    - y_sol: the reaction fluxes
    - alpha_sol: the thermodynamic force associated with each reaction
    - n_sol: the log metabolite counts :math:`\log( N_{avagadro}) + \log(cell_{Volume}) + \log(counts )`
    """
    # Flip Stoichiometric Matrix
    S_T = S    # S_T is the Stoich matrix with rows as reactions, columns as metabolites
    ...
    def steady_state_metab(m,j):
        """steady state metabolite constraint."""
    ...
    def relaxed_reg_cns_upper(m,i):
        """Relaxed upper bound constraints regulating reaction."""
        return ( m.b[i] - pe.log(m.K[i]) ) >= m.h[i] - Mb*(m.u[i])
    m.rxr_cns_up = pe.Constraint(m.react_idx,rule = relaxed_reg_cns_upper)


    def relaxed_reg_cns_lower(m,i):
        """Relaxed lower bound constraints regulating reaction."""
        return ( m.b[i] - pe.log(m.K[i]) ) <= m.h[i] + Mb*(1 - m.u[i])
    m.rxr_cns_low = pe.Constraint(m.react_idx,rule = relaxed_reg_cns_lower)
    ...
    def sign_constraint(m,i):
        """Constraint that flips depending on the reaction direction"""
        return (pe.log(m.K[i]) - m.b[i])*m.y[i] >= 0
    m.sign_y_cns = pe.Constraint(m.react_idx, rule = sign_constraint)


    def y_sign_relax(m,i):
        """Relaxed flux constraint on the sign."""
        return 2*m.u[i] - 1 == (m.y[i]/(abs(m.y[i]) + 1e-50) )
    m.y_sign_relax_cns = pe.Constraint(m.react_idx,rule = y_sign_relax)
    ...
    def M_lower_cnstrnts(m,i):
        """Variable metabolite lower bounds."""
        return  m.VarM[i] >= VarM_lbnd
    m.VarM_lb_cns = pe.Constraint(m.VarM_idx,rule = M_lower_cnstrnts)
    ...
    ## Solve the Model
    status_obj = opt.solve(m, options=solver_opts, tee=True)
